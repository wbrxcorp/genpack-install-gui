...
- **アプリ内ターミナル** — PTY 上のシェルを Slint の Image に描画する組み込み端末（別クレート [slint-terminal](https://github.com/wbrxcorp/slint-terminal)）。GUI を離れずにトラブルシュート用のシェル操作ができます。`exit` または Ctrl-D で GUI に戻ります。
...
| オプション | 説明 |
|---|---|
| `--wait-for-display` | 接続済みディスプレイが現れるまでブロックしてから Slint を初期化します。KMS 自動起動時に、DRM デバイス出現後・ディスプレイ（HDMI EDID）の認識完了前に起動して即クラッシュするのを防ぐためのものです。`/sys/class/drm` を 0.5 秒間隔でポーリングし、`connected` なコネクタと有効モードが揃うまで待機します。systemd から自動起動する構成で指定します。 |

`mock` はビルド時 feature（`cargo build --features mock`）であり、実行時オプションではありません。

動作に影響する主な環境変数:

| 環境変数 | 説明 |
|---|---|
| `SLINT_BACKEND` | 描画バックエンドの明示指定（`linuxkms` / `winit`）。未指定なら Wayland/X セッションの有無で自動選択します。 |
| `LIBSEAT_BACKEND` | `builtin` で libseat が DRM/入力を直接オープン（root 実行時）、`seatd` で seatd 経由。 |
| `SLINT_SCALE_FACTOR` | スケールファクターを明示指定します。指定時は KMS 物理解像度からの自動スケール決定を行わず、この値を優先します。 |
| `LANG` | `ja` で始まる場合は日本語 UI、それ以外は英語 UI を選択します。 |
...
| システム情報画面 | 実装済み |
| 多言語対応（日本語 / 英語） | 実装済み |
| アプリ内ターミナル画面 | 実装済み（PTY + [slint-terminal](https://github.com/wbrxcorp/slint-terminal) を Slint の Image に描画） |
